...
Based on Panoramio Rest API we prepare the most popular photos for each city. 
The map uses Google's Static Map API. 
Every day the new random city(ies) will be tweeted via Rake task and Twitter API.
...
* get rails (and all realated).

* run bundle install to get all gems required for currect work.

* set up a scheduled task to run Rake task tweet.rake[lib/tasks/tweet.rake]
